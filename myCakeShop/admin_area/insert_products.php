...
if (isset($_POST['insert_product'])) {
    ...
    if (empty($product_title) || empty($product_description) || empty($product_keywords) || empty($product_category) || empty($product_price) || empty($product_image1)) {
        echo "<script>alert('Please fill all the available fields')</script>";
        exit();
    } else {
        $upload_directory = "./product_IMGs/";
        $target_image1 = $upload_directory . basename($product_image1);

        if (move_uploaded_file($temp_image1, $target_image1)) {
            // Insert query
            $insert_products = "INSERT INTO `products` (product_title, product_description, product_keywords, category_id, product_image1, product_price, date, status) VALUES ('$product_title','$product_description','$product_keywords','$product_category','$product_image1','$product_price', NOW(), '$product_status')";
            ...
            <div class="col-lg-6 col-xl-4 my-3 col-md-6">
                <form action="insert_product.php" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-outline mb-2">
                        ...
                        <input type="text" id="product_keywords" class="form-control" name="product_keywords" placeholder="Enter product keywords" autocomplete="off" required="required"/>
                    </div>
                    <!-- Category Select -->
                    <div class="form-outline mb-2">
                        <label for="product_category" class="form-label fw-bold">Select a Category</label>
                    ...
                    <input type="submit" name="insert_products" class="btn btn-info px-4 mb-4" value="Insert Products"/>
                </form>
            </div>
        </div>
    </div>
